...
export interface MyListingStaysContainerProps {
  stays: StrapiPaginatedResult<Stay>;
}

const MyListingStaysContainer: FC<MyListingStaysContainerProps> = ({
  stays,
}) => {
  const listingStays = stays.data;
  ...
  const renderSection1 = () => {
    return (
      <div className="listingSection__wrap">
        <div>
          <h2 className="text-2xl font-semibold">Your Listing Stays</h2>
        </div>
        <div className="w-28 border-b border-neutral-200 dark:border-neutral-700"></div>

        <div>
          <div className="mt-8 grid grid-cols-3 gap-6 md:gap-7 sm:grid-cols-1">
            {listingStays
              .filter((_, i) => i < 4)
              .map((stay) => (
                <StayCard2
                  key={stay.id}
                  data={{ ...stay.attributes, id: stay.id }}
                />
              ))}
          </div>
...
export default MyListingStaysContainer;
